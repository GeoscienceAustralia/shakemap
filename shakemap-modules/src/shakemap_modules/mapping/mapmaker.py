...
from esi_shakelib.plotting.contour import contour, getContourLevels
from shakemap_modules.utils.utils import get_object_from_config
from shakemap_modules.mapping.mercatormap import MercatorMap
from shakemap_modules.mapping.scalebar import draw_scale
...
def _draw_imt_legend(
    ...
    Args:
        ...
        palette (ColorPalette): ColorPalette using range of input data and
            IMT_CMAP.
        imtype (str): One of 'PGV','PGA','SA(0.3)',etc.
        _gmice (GMICE object): The GMICE used for this map.
    ...
    if _gmice is None or not _gmice.supports(imtype):
        _gmice = WGRW12()
    if _gmice.supports(imtype):
...
def _draw_mmi_legend(
    fig, palette, _gmice, process_time, map_version, point_source, tdict
):
    ...
    pga_values, _ = _gmice.getGMfromMI(mmi_centers, imt.from_string("PGA"))
    pgv_values, _ = _gmice.getGMfromMI(mmi_centers, imt.from_string("PGV"))
...
def draw_map(adict, override_scenario=False):
    ...
    # mmimap = ColorPalette.fromFile("/Users/cbworden/sm_colors2.cpt")

    # z0 = np.linspace(1, 10, 100)
    # z1 = np.linspace(1.01, 10.01, 100)
    # mmimap = ColorPalette.fromColorMap(
    #     #        "plasma", z0, z1, mpl.colormaps["plasma_r"]
    #     "custom",
    #     z0,
    #     z1,
    #     cmr.custom,
    ...
    config = adict["config"]
    try:
        _gmice = get_object_from_config("gmice", "modeling", config)
        ...
        gmice_imts = [
            imt.__name__ for imt in _gmice.DEFINED_FOR_INTENSITY_MEASURE_TYPES
        ]
        gmice_pers = _gmice.DEFINED_FOR_SA_PERIODS
...
def draw_uncertainty_map(adict, key, override_scenario=False):
    ...
    # norm = colors.Normalize(vmax=np.ceil(np.max(pimtdata)),
    #                         vmin=np.floor(np.min(pimtdata)))
